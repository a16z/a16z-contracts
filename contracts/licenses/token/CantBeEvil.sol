...
abstract contract CantBeEvil is ERC165, ICantBeEvil {
    using Strings for uint;
    string internal constant _BASE_LICENSE_URI = "ar://_D9kN1WrNWbCq55BSAGRbTB4bS3v8QAPTYmBThSbX3A/";

    function getLicenseURI(uint256 _tokenId) public view returns (string memory) {
        LicenseVersion licenseVersion = _getLicenseVersion(_tokenId);
        return string.concat(_BASE_LICENSE_URI, uint(licenseVersion).toString());
    }

    function getLicenseName(uint256 _tokenId) public view returns (string memory) {
        LicenseVersion licenseVersion = _getLicenseVersion(_tokenId);
